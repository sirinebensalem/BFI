      ...
      }
      {
        completeBoardSelected?.columns.length !== 0 && (
          <div className=' h-full flex p-6 overflow-x-scroll scrollbar'>
                ...
                completeBoardSelected?.columns.map((col, index) => (
                  <div key={col.id} className='min-w-[280px] w-[280px] flex flex-col mr-6'>
                    <div className='h-[15px] flex items-center mb-6'>
                      ...
                      <p className='font-bold text-gray-800 tracking-S text-bL'>{col.name} ({col.tasks?.length})</p>
                    </div>
                    <Droppable droppableId={col.id} type={"COLUMN"}>
                      {(droppableProvided, droppableSnapshot) => (
                        <div  className={`w-full flex flex-col rounded-2xl p-3 shadow-2xl  ${
                          droppableSnapshot.isDraggingOver
                            ? `bg-green-200`
                            : " bg-slate-200 " // Add your normal styling here
                        }`}
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white text-green-400 hover:bg-green-100 hover:text-green-500 rounded-lg focus:outline-none"
                >
            ...
            <div>
      <button
        onClick={() => setDisplayAddEditBoard({ display: true, mode: 'EDIT' })}
        className="flex items-center gap-2 px-4 py-2 bg-white text-gray-400 hover:text-gray-500 rounded-lg focus:outline-none transition duration-300"
      >
        <PlusCircleIcon className="h-6 w-6" />
